...
const Breadcrumbs = ({ crumbs, centered }) => {
  ...
  const crumbList = useMemo(() => {
    ...
    return crumbs.reduce(
      (acc, item, index) => (index === 0 ? [item] : acc.concat({ index, text: '/' }, item)),
      [],
    );
    ...
    <div className={styles.container}>
      <div className={`${styles.breadcrumbs} ${centered ? styles.centered : ''}`}>
        {crumbList.map((item) => {
          if (item?.text === '/') {
            return (
              <span key={item.index} className={styles.breadcrumbs__separator}>
                <Image
          ...
          if (typeof item === 'string') {
            return (
              <span key={item} className={styles.breadcrumbs__text}>
                {item}
              </span>
            );
          }
